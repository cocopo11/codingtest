...
접두어 관계가 있는 전화번호들을 쉽게 비교하기 위해 먼저 전화번호 목록을 정렬했다.

문자열 기준으로 정렬하면 앞부분이 비슷한 전화번호들이 서로 가까이 모이게 되고 한 전화번호가 다른 전화번호의 접두어라면 더 짧은 문자열이 앞쪽에 오게 된다.

따라서 모든 전화번호 쌍을 비교할 필요 없이 정렬된 목록에서 서로 인접한 두 전화번호만 확인하면 된다.

순회하면서 뒤쪽 전화번호가 앞쪽 전화번호로 시작하는지

```python
pb[i + 1].startswith(pb[i])
```

를 이용해 확인한다.

접두어가 발견되면 바로 `False`를 반환하고 끝까지 발견되지 않고 순회를 마치면 `True`를 반환한다.
...
전화번호의 개수를 `n`이라고 하고 전화번호 하나의 길이는 문제 조건상 최대 20이기 때문에 문자열 비교 비용은 상수 시간으로 볼 수 있다.

먼저 `pb.sort()`를 수행하므로 정렬에 `O(n log n)`이 필요하다.

이후 인접한 전화번호를 한 번씩 비교한다. `startswith()`도 최대 20개의 문자만 확인하므로 각 비교는 `O(1)`로 볼 수 있고 전체 순회는 `O(n)`이다.

따라서 전체 시간복잡도는 `O(n log n) + O(n) = O(n log n)`

### 공간복잡도

별도의 큰 자료구조를 추가로 만들지는 않는다.

다만 Python의 `list.sort()`는 내부 정렬 과정에서 보조 메모리를 사용할 수 있으므로
구현 관점에서는 추가 공간복잡도를 최악의 경우 `O(n)` 정도로 볼 수 있다.

### 배운 점

처음부터 모든 전화번호 쌍을 비교하면 비교 대상이 너무 많아질 수 있지만 정렬을 먼저 수행하면 앞부분이 비슷한 번호들이 서로 인접하게 된다.

즉, 정렬을 단순히 순서를 맞추는 용도가 아니라
비교해야 할 후보를 줄이는 전처리 과정으로 사용할 수 있다는 점을 배웠다.

또한 `startswith()`를 사용하면 문자열이 특정 접두어로 시작하는지
직관적으로 확인할 수 있다.
...
먼저 논문의 인용 횟수를 오름차순으로 정렬했다.

```python
c = sorted(citations)
```

정렬된 배열에서 `i`번째 위치를 기준으로 보면,
현재 위치부터 마지막까지 남아 있는 논문의 개수는
...
배열이 정렬되어 있으니까 현재 값 `c[i]`가 `h` 이상이라면 `i`번째 논문부터 마지막 논문까지의 모든 값도 최소 `h` 이상이다.
...
`i`가 증가할수록 `h = len(c) - i`는 점점 작아진다.

따라서 가장 큰 `h`부터 순서대로 검사하게 되고,
처음 조건을 만족한 `h`가 가능한 최대 H-Index가 된다.

끝까지 조건을 만족하는 값이 없다면 H-Index는 `0`이므로 `0`을 반환했다.
...
그 다음 정렬된 배열을 최대 한 번 전체 순회하므로 `O(n)`이 필요하다.
...
`sorted()`는 원본 리스트를 수정하지 않고 새로운 리스트를 반환한다.

따라서 정렬된 결과를 저장하기 위해 `O(n)`의 추가 공간이 필요하다.

### 배운 점

정렬을 이용하면 현재 위치보다 뒤에 있는 값들이 모두 현재 값 이상이라는 것을 활용할 수 있다.

이번 문제에서는 매번 "현재 값 이상인 논문이 몇 개인가"를 직접 세지 않고,
정렬된 배열의 인덱스를 이용해
...
만으로 현재 위치부터 남아 있는 논문의 개수를 바로 구할 수 있었다.

또한 `h`를 큰 값부터 작은 값으로 확인하고 있기 때문에,
처음 조건을 만족한 순간 바로 반환해도 그 값이 최대 H-Index가 된다는 점을 알 수 있었다.
